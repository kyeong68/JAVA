...
//		nums[4] = 20;
//		nums[5] = 5;
//		System.out.println(nums[3]);//10정답
//	
...
//		}
//		
////		//for-each : 확장for문-> 배열에 있는 모든 값을 꺼내는 것.
////		for(잠깐담아둘변수:배열) {
////		실행문장
////		
////		
...
//---------------------------------------------------------
	 //10칸짜리 배열에 랜덤한 수를 넣고,
	 //해당하는 수의 총합과 평균을 구하기. /최대값/ 최소값/ 홀수 개수/ 짝수개수
		
	// 1.10칸짜리 배열 만들기
		int[] nums = new int[10];
//		String[] names = new String[10];
		Random rd = new Random();//ctrl+shift+o ->자동 import
	...
	//총합과 평균을 구하기. /최대값/ 최소값/ 홀수 개수/ 짝수개수
	int sum =0;
	double avg =0;
	...
	for(int i=0;i<nums.length;i++) {
		...
		if(min > nums[i]) {//지금 현재의 최소값과 배열에 있는 값 비교
			min = nums[i];
